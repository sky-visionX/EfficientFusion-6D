    ...
    def __getitem__(self, index):
        ...
        if self.add_noise:
            ...
            for k in range(5):                
                ...
                f_label = np.array(Image.open('{0}/{1}-label.png'.format(self.root, seed))) #label像素值代表的是类别号
        ...
        while 1:
            ...
            # plt.imshow(mask_label, cmap='gray')
        ...
        # plt.subplot(2, 3, 1)
        # plt.imshow(np.transpose(img, (2, 1, 0)), cmap='gray')
        # plt.subplot(2, 3, 2)
        # plt.imshow(np.transpose(img_masked, (2, 1, 0)), cmap='gray')
        # plt.subplot(2, 3, 3)
        # plt.imshow(label, cmap='gray')
        # plt.show()
