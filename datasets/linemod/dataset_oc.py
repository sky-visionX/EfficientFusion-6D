...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num, add_noise, root, noise_trans, refine):
        ...
        '''
        self.objlist = [1, 5, 6, 8, 9, 10, 11, 12]  # 这里表示目标物体类别序列号
        self.mode = mode
        ...
        input_file = open('{0}/test.txt'.format(self.root))
        for item in self.objlist: # 对每个目标物的相关数据进行处理
            while 1:
                ...
                self.list_rgb.append('{0}/RGB-D/rgb_noseg/{2}.png'.format(self.root, input_line))
                self.list_depth.append('{0}/RGB-D/depth_noseg/{2}.png'.format(self.root, input_line))
                if self.mode == 'eval':
                    self.list_label.append('{0}/masks/{1}/{2}.png'.format(self.root, '%02d' % item, input_line))
                else:
    ...
    def __getitem__(self, index):
        ...
        if obj == 2: # 物体2里面的gt包含多目标
            for i in range(0, len(self.meta[obj][rank])):
                if self.meta[obj][rank][i]['obj_id'] == 2: # obj代表图片文件夹，rank代表图片名，obj_id代表该图片内的类别
                    meta = self.meta[obj][rank][i] # 此处意思只取目标物2的gt，当成和其他文件夹一样，一张图只有单类来处理
                    break
        else:
